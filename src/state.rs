...
impl From<Orientation> for image::metadata::Orientation {
    fn from(value: Orientation) -> Self {
        match value {
            Orientation::Portrait0Deg => Self::NoTransforms,
            Orientation::Landscape90Deg => Self::Rotate90,
            Orientation::Portrait180Deg => Self::Rotate180,
            Orientation::Landscape270Deg => Self::Rotate270,
        }
    }
}
...
impl State {
    ...
    pub fn cover_image(&self) -> Option<Rc<DynamicImage>> {
    ...
    fn regen_cover_image(&mut self) {
        ...
        let Ok(mut img) = cover::retrieve_cover(book).inspect_err(|err| {
            eprintln!("Retrieving cover image failed: {err}");
        }) else {
            return;
        };
        img.apply_orientation(self.orientation.into());
        self.cover_image = Some(Rc::new(img));
        self.regen_cover_image_buf();
    }

    fn regen_cover_image_buf(&mut self) {
        self.cover_image_buf.take();
        ...
        let buf = img
            ...
            .into_luma8();
        self.cover_image_buf = Some(Rc::new(buf));
    }
}
